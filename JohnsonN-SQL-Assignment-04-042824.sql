...
SELECT AddressLine1, City, StateProvince, CountryRegion, PostalCode
FROM SalesLT.Address
WHERE CountryRegion = 'Canada' AND City in ('Burnaby', 'Vancouver') 
AND StateProvince = 'British Columbia'
ORDER BY StateProvince ASC, City ASC;
...
SELECT AddressLine1, City, StateProvince, CountryRegion, PostalCode
FROM SalesLT.Address
WHERE City IN ('Victoria', 'Vancouver') AND StateProvince = 'British Columbia'
ORDER BY City ASC;
...
--		YOUR ANSWER: 3399.99	
SELECT Name, ProcutNumber, Size, ROUND(CAST(ListPrice AS MONEY), 2) AS ListPrice,
ROUND(CAST(StandardCost AS MONEY), 2) AS StandardCost
...
SELECT Name, ProductNumber, Size, ListPrice
FROM SalesLT.Product
WHERE SIZE IN ('S', 'M')
...
SELECT Name, ProductNumber, Size, ROUND(CAST(StandardCost AS MONEY), 2) AS StandardCost,
ROUND(CAST(ListPrice AS MONEY), 2) AS ListPrice, Weight
FROM SalesLT.Product
...
--		YOUR ANSWER: 1(11)500 555-0193	
SELECT CompanyName, phone
FROM SalesLT.Customer
WHERE Phone LIKE '%19%'
ORDER BY Phone ASC;
/*
This only returns 57 rows, but follows the same formula as following pattern matching queries
Checked query in book and with ChatGPT and this seems like the proper query, not sure what I'm missing
*/
